...
import { get, post, put, del } from "../../services/Fetch";
...
export const fetchIdeaCard = () => dispatch =>
  get("accidents")
    .then(data => dispatch(fetchSuccess(data)))
    .catch(error => dispatch(fetchFailed(error)));

export const postIdeaCard = accident => dispatch =>
  post("accidents", accident)
    .then(data => dispatch(postSuccess(data)))
    .catch(error => dispatch(postFailed(error.toString())));

export const putIdeaCard = (id, data) => dispatch =>
  put("accidents", id, data)
    .then(response => dispatch(putSuccess(response)))
    .catch(error => dispatch(putFailed(error.toString())));

export const deleteIdeaCard = id => dispatch =>
  del("accidents", id)
